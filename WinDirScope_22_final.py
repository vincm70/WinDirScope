...
# 1.4.4 :
# - Chemin de l'analyse en bleu foncé, cliquable
# - Envoi mail via PowerShell + Outlook (2016+ Win32) avec message propre si Outlook absent
# - Fenêtre "À propos" dédiée avec lien cliquable vers le partage ChatGPT
APP_VERSION = "1.4.4"
...
class WinDirScopeApp:
    ...
    def _build_ui(self):
        ...
        # Style : chemin en bleu foncé
        style = ttk.Style(self.master)
        style.configure("StatusLabel.TLabel", foreground="#003366")

        # Label de statut (chemin cliquable)
        self.lbl_status = ttk.Label(
            top_frame,
            text="Aucun dossier analysé.",
            style="StatusLabel.TLabel",
            cursor="hand2",
        )
        self.lbl_status.pack(side=tk.LEFT, padx=10)
        self.lbl_status.bind("<Button-1>", self.on_status_click)
    ...
    def on_about(self):
        """Fenêtre À propos avec lien cliquable vers le partage ChatGPT."""
        win = tk.Toplevel(self.master)
        win.title("À propos de WinDirScope")
        ...
        ttk.Label(
            frm,
            text="WinDirScope - Analyse d'espace disque",
            font=("Segoe UI", 10, "bold"),
        ).pack(anchor="w")

        ttk.Label(frm, text=f"Version : {APP_VERSION}").pack(anchor="w")
        ttk.Label(frm, text="Auteur : Vincent TOUZOT").pack(anchor="w")
        ttk.Label(
            frm,
            text=(
        ...
        ttk.Label(frm, text="Fonctions principales :", font=("Segoe UI", 9, "bold")).pack(
            anchor="w"
        )
        ttk.Label(
            frm,
            text=(
                ...
                "- Indication des dossiers en accès refusé."
            ),
            justify="left",
        ).pack(anchor="w", pady=(0, 8))

        # Lien cliquable vers le partage ChatGPT
        link_url = "https://chatgpt.com/share/691b8c21-9824-8008-ad38-d2ac342c0379"
        link_lbl = ttk.Label(
            frm,
            text="Projet détaillé (conversation ChatGPT)",
            foreground="#0000EE",
            cursor="hand2",
        )
        link_lbl.pack(anchor="w", pady=(4, 0))

        def open_link(event=None, url=link_url):
            try:
                webbrowser.open(url)
            except Exception:
        ...
        ttk.Button(btn_frame, text="Fermer", command=win.destroy).pack(anchor="e")
    ...
    def on_status_click(self, event):
        """
        Clic sur le label de statut :
        - pendant l'analyse : ouvre le dossier en cours d'analyse
        - après l'analyse : ouvre le dossier racine analysé
        """
        path = None
        if self.scan_running and self.current_scan_path:
            path = self.current_scan_path
        elif self.root_node:
            path = self.root_node.path

        if path and Path(path).exists():
            open_file_in_default_app(Path(path))
