...
def open_folder_in_explorer(path: Path):
    ...
    try:
        ...
        # On ignore les erreurs d'ouverture, ce n'est pas bloquant
        pass
...
class WinDirScopeApp:
    ...
    def on_export_results(self):
        ...
        # Nom de base par défaut : date + nom du dossier analysé, en HTML par défaut
        ...
        # Ouvrir automatiquement le dossier d'export dans l'explorateur
        if exported_files:
            open_folder_in_explorer(exported_files[0])
    ...
    def _export_html(self, filepath: Path):
        """Export en page HTML avec dossiers repliables/dépliables et filtres."""
        def esc(s: str) -> str:
            return (
                s.replace("&", "&amp;")
                 .replace("<", "&lt;")
...
(function() {
  ...
  function applyFilters() {
    ...
    nodes.forEach(function(el) {
      ...
      var container = el.closest('details');
      if (!container) {
        container = el;
      }
