...
class DiskXplorerApp:
    def __init__(self, master: tk.Tk):
        self.master = master
        self.master.title("DiskXplorer - Analyse d'espace disque")
    ...
    def on_export_results(self):
        ...
        default_name = f"DiskXplorer_{timestamp}_{root_name}.csv"
    ...
    def _export_tree_csv(self, filepath: Path):
        ...
        with filepath.open("w", newline="", encoding="utf-8-sig") as f:
            ...
            writer.writerow(["Chemin complet", "Nom", "Niveau", "Type", "Taille (octets)", "Taille lisible", "% du total"])
            for row in rows:
                writer.writerow([
                    row["path"],
                    row["name"],
